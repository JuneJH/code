...
function flatten(arr, shallow, strict, output) {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i] instanceof Array) {
            if (shallow) {
                arr[i].forEach(ele => output[index++] = ele)
            } else {
...
function addLinkValueByNode(root, value, node) {
    ...
    if (root.value == node.value) {
        const nowNode = new CreateLink(value);
        nowNode.next = root.next;
        root.next = nowNode;
        return nowNode;
    } else {
...
function prim(distance,point,start){
    ...
    function _getDistance(distance,point){
        ...
        for(let i = 0; i < horde.length; i ++){
            let position = point.indexOf(horde[i]);
            for(let j = 0; j < distance[position].length; j ++){
                if(!horde.includes(point[j]) && distance[position][j] < result.distance){
                    result.from = horde[i];
                    result.to = point[j];
...
function add(a,b){
    ...
    for(let i = aArr.length - 1; i >= 0;i --){
        const re = parseInt(aArr[i]) + parseInt(bArr[i]) + flag;
        if(re > 10){
            result[i] = re % 10;
            flag = 1
        }else{

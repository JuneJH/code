...
// 16. 通过下标设置链表中的某一个数据 setLinkValueBySubscript  setLinkValueBySubscript(a, 'updateValue', 3))
function setLinkValueBySubscript(root, value, subscript, index = 0) {
    ...
    if (subscript == index) {
        ...
        return setLinkValueBySubscript(a, value, subscript, ++index)
    }
}
...
function prim(distanceSet,point,start){
    ...
    function _getDistance(distanceSet,point){
        ...
        for(let i = 0; i < horde.length; i ++){
            ...
            for(let j = 0; j < distanceSet[position].length; j ++){
                if(!horde.includes(point[j]) && distanceSet[position][j] < result.distance){
                    result.from = horde[i];
                    result.to = point[j];
                    result.distance = distanceSet[position][j]
                }
            }
        }
